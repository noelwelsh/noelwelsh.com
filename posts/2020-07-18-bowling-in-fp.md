...
I recently led a training session where we implemented the [rules for scoring ten-pin bowling][bowling-case-study] in Scala. This is an interesting case study. It's small enough that you can pick up the rules in a few minutes, but the dependencies between frames makes calculating the score no trivial. I found it interesting enough that I implemented [my own solution][implementation] and decided to write up my process here. 
...
It we want to calculate the length of a list we can start with the skeleton
...
With this all done the scoring algorithm is a structural recursion over the `Frames` and `FinalFrame` that make up a game, as well as the `State`. [The code is lengthy][score], but it isn't hard to write and a good deal of it is generated by the IDE (in my case, Metals with Doom Emacs.)
...
I defined a few different [generators][generators] for the various types of frames, which enabled me to [test][gamespec] both the examples given in the instructions and examples generated at random. I found quite a few errors with these tests, both in my scoring algorithm and in how I was generating data. Luckily they were all very easy to diagnose. As the scoring algorithm was very explicit it was easy to work out what I had done wrong (which was usually forgetting to include a roll somewhere).
