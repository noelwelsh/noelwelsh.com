...
One of the most fun projects I've worked on was creating an algorithm for generating unique identifiers.
In this post I'll walk through my reasoning,
and show how a combination of mathematics, computer science, and business requirements lead to the implementation.
...
The system shared identifiers with a third-party, which only supported identifiers of up to 20 alphanumeric characters.
...
we can represent it as a type 8 UUID, 
allowing us to interoperate with existing systems.
This seems like a great solution, but is 103 bits going to be enough?
...
purely random data makes for an acceptable generation scheme
...
Using a timestamp takes away some bits from random data.
...
the bits we lose to a timestamp can be made up for by the decreased probability of a collision arising from the smaller number of identifiers in the timestamp window.
...
so in this case we could choose to use timestamps.
...
For example, the rate of generation will usually vary according to the time of day, 
...
so just have to make sure we make conservative assumptions that will hold no matter what happens.
...
There is nothing here that isn't covered in a standard CS curriculum&mdash;basic probability and combinatorics, and binary representations and bit manipulation&mdash;and yet it seems in practice these skills are not common.
