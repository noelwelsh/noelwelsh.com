...
I've been using static site generators for a very long time. I have used at least Jekyll, Hakyll, Gatsby, Eleventy, Next.js, and Zola to create personal and commercial sites. In fact one of the first commercial programs I wrote was a static site generator. That generator didn't live for very long but it shows I've been thinking, on and off, about this problem for a while. I've also been frustrated with the limitations of existing systems starting about ten minutes after I finished that first one way back in 1997. I hope to show that with some simple changes, which is mostly come down to a change of perspective, much better tools are possible.


## Static Site Generators

The core problem solved by static site generators is to create a collection of text files---usually HTML files making up a website---from several source components. A typical example is to combine a blog post with a template that styles the post. The default in almost every current SSG is to write blog posts in [Markdown][commonmark] format with [YAML][yaml] "front-matter". Front-matter is essentially a set of key-value pairs that add additional information, such as the title and author, to the post. Here's an example from my blog, where the front matter defines the title of the blog post.
...
Notation is not considered very important in most programming language research. However it becomes central when you consider programming as a system, as notation is the user interface by which programming is accessed. The text of the web pages is the majority of the user generated content in an static site generator, so it is correctly made the easiest thing to create. 


## Programming Matters

If static site generators get notation right, the majority of them get almost everything else about programming wrong. Here I'm going to look at the sorry state of programming in the static site generators.

Functions are a basic mechanism for abstraction. The concept of a function is straight-forward. Informally, a function accepts some stuff (it's parameters) and returns something back when arguments are applied to the parameters. Changing the arguments gives different results, and hence function can abstract over some common pattern.

We are going to see how [Liquid][liquid] treats functions. Liquid is perhaps the most commonly used templating language as it's the one used by [Jekyll][jekyll], the default SSG on Github. The functionality reported below was tested using Ruby 2.7.2p137 and Jekyll 4.2.0.

Liquid doesn't have functions as such, but it does have filters, templates, and tags.

A filter is the Liquid feature most like a function. It has unusual syntax, using a pipe operator rather than standard function application. For example, instead of writing `abs(-4)` in Liquid one writes `-4 | abs`. Pipes can be nested, and if a filter takes more than one argument the successive arguments follow the filter after a colon. For example `-4 | abs | at_least: 42` evaluates to `42` calling first the filter `abs` and then `at_least`. The `at_least` filter has two parameters. 
...
Here's an example of a simple include, which we'll assume is stored in a file named `header.html`
...
This include has a single parameter called `title`, and hence we can consider templates as functions that produce text. Functions, that is, with a flaw: they are dynamically scoped[^scope]. Includes share the scope of the template that includes them, and can directly reference (and even modify!) any variables that are present in the template. This makes parameters more a cosmetic than essential feature. Dynamic scoping was shown to be a Bad Things in the [the Art of the Interpreter][aoti] back in 1973, so it's unfortunate to find it in a language first released in 2006.

[^scope]: I'm not certain if Liquid has dynamic scope or just one global scope shared by all templates and includes, which is slightly different. It didn't do the experiments to confirm one way or the other because, dear reader, I am utterly sick of Liquid at this point in time.
...
My experience using these frameworks is that simple changes, which I expect to take a maybe half an hour, turn into marathon multi-hour sessions. The arbitrary limitations of the abstraction mechanisms mean I often cannot implement a feature in the way I expect. The ad-hoc semantics mean that knowledge from other programming languages, or even other features in the same language, can't be used to infer to the behaviour of unfamiliar features. Finally the poor error messages give little clue as to what is wrong, and there is no debugger or REPL to give clarity.

These problems are largely solved. I've already mentioned dynamic scoping, which was addressed in 1973. Escaping into and out of a data format is solved in Lisp using quasi-quote and unquote, and in other languages using string interpolation. Semantics is clearly a major concern in programming language theory, and finding minimal expressive abstractions arguably dates back to the lambda calculus, developed before digital computers even existed.


## Staging is Real

There is a new class of static site generators, that output sites that are rendered using Javascript instead of plain text. Examples include [Gatsby][gatsby] and [Next.js][next]. These tools have some issues---for one, the site may not be viewable without Javascript running. However they solve most of the issues I gave above. I don't think this is because the authors have any deeper knowledge of programming languages, but rather because the assumption is code will be written in Javascript which, despite it's many flaws, is a reasonably well-designed and modern language in its current incarnation.
...
I hope that future authors of static site generators might read this article and learn some of the lessons of the systems that have come before them. 

I also hope that those who work on programming languages might find here support for the validity of the traditional concerns of programming language theory, and see the need to expand the vision to include the context in which programming takes place.



I think it's clear that I find current static site generators rather frustrating. The overall problem is, I think, the view that a static site generator is a collection of random bits and pieces and not a coherent programming system. Nobody ever took a step back and asked, say, what is the essence of filters, templates, and tags? Instead each problem the authors encountered was treated as distinct and hence confusing, overlapping abstractions grew.

I believe every time someone interacts with a computer they are programming. Often not in way that is interesting to those who study programming, but even something as mundane as a word processor benefits

I also think the flaws could be easily overcome. Abstracting away the specifics of SSGs, the larger point is this: in my mind programming languages do two things, control the machine and provide a notation. 
